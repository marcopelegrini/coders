    ...
    coders.changeHosts.hostesseTemplateManager = {
    	...
    	_parse: function(fileName, rootDir, includePath){
			...
			for(var i = 0;i < lines.length;i++){
				...
				if (match){
					var includeFilePath = match[1].trim();
					includeFilePath = includeFilePath.replace(this.dirSeparatorPattern, this.fileSeparator);
					var include = rootDir + this.fileSeparator + includeFilePath  + this.defaultHostFileExtension;
					lineValue = this._parse(include, rootDir, includeFilePath);
				}
