    ...
    coders.changeHosts.options.definitions = {
        ...
        loadTree: function(tree){
            try{
                ...
                this.ctx.logUtils.info("Found " + this.hostMap.size + " hosts dir(s).");
                
                if(this.ctx.logUtils.isTrace()){
                    for (var [dir, hosts] of this.hostMap) {
                        this.ctx.logUtils.trace('Host dir found: ' + dir);
                        for (var i = 0; i < hosts.length; i++) {
                            this.ctx.logUtils.trace('Host file found: ' + hosts[i].fileName + ' on path ' + hosts[i].path);
                        }
                    }
                }
        ...
        readDir: function(path, father, isRoot) {
            ...
            while (entries.hasMoreElements() && this.fileCount < this.readFilesLimit) {
                ...
                if (entry.isFile()) {
                    this.fileCount++;
                    this.ctx.logUtils.debug('Read files limit: ' + this.readFilesLimit + ' files read: ' + this.fileCount);
